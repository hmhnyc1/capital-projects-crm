...
const createRLSPolicies = [
  ...
  `CREATE POLICY IF NOT EXISTS "Users can view own bank statements" ON bank_statements_detailed FOR SELECT USING (auth.uid() = user_id)`,
  `CREATE POLICY IF NOT EXISTS "Users can insert own bank statements" ON bank_statements_detailed FOR INSERT WITH CHECK (auth.uid() = user_id)`,

  // mca_positions_detailed
  `CREATE POLICY IF NOT EXISTS "Users can view own MCA positions" ON mca_positions_detailed FOR SELECT USING (auth.uid() = user_id)`,
  `CREATE POLICY IF NOT EXISTS "Users can insert own MCA positions" ON mca_positions_detailed FOR INSERT WITH CHECK (auth.uid() = user_id)`,

  // underwriting_scorecards_detailed
  `CREATE POLICY IF NOT EXISTS "Users can view own scorecards" ON underwriting_scorecards_detailed FOR SELECT USING (auth.uid() = user_id)`,
  `CREATE POLICY IF NOT EXISTS "Users can insert own scorecards" ON underwriting_scorecards_detailed FOR INSERT WITH CHECK (auth.uid() = user_id)`,

  // risk_flags_detailed
  `CREATE POLICY IF NOT EXISTS "Users can view own risk flags" ON risk_flags_detailed FOR SELECT USING (auth.uid() = user_id)`,
  `CREATE POLICY IF NOT EXISTS "Users can insert own risk flags" ON risk_flags_detailed FOR INSERT WITH CHECK (auth.uid() = user_id)`,

  // documents_detailed
  `CREATE POLICY IF NOT EXISTS "Users can view own documents" ON documents_detailed FOR SELECT USING (auth.uid() = user_id)`,
  `CREATE POLICY IF NOT EXISTS "Users can insert own documents" ON documents_detailed FOR INSERT WITH CHECK (auth.uid() = user_id)`,
]

export async function setupDetailTables() {
  console.log('[setupDetailTables] 🔍 Starting setup...')
  ...
  try {
    ...
    console.error('[setupDetailTables] ❌ Error:', error instanceof Error ? error.message : String(error))
