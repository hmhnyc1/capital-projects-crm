...
const migrationSQL = `-- Bank Statements Detail Table
...
-- RLS Policies
CREATE POLICY "Users can view own bank statements" ON bank_statements_detailed FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Users can insert own bank statements" ON bank_statements_detailed FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can view own MCA positions" ON mca_positions_detailed FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Users can insert own MCA positions" ON mca_positions_detailed FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can view own scorecards" ON underwriting_scorecards_detailed FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Users can insert own scorecards" ON underwriting_scorecards_detailed FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can view own risk flags" ON risk_flags_detailed FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Users can insert own risk flags" ON risk_flags_detailed FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can view own documents" ON documents_detailed FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Users can insert own documents" ON documents_detailed FOR INSERT WITH CHECK (auth.uid() = user_id);`

export async function POST(request: NextRequest) {
  try {
    // Allow one-time migration execution
    // In production, this would be restricted to admin users
    const isLocalhost = request.headers.get('host')?.includes('localhost')
    const hasSecret = request.headers.get('x-migration-secret') === process.env.MIGRATION_SECRET

    if (!isLocalhost && !hasSecret) {
      // Still allow execution for this one-time setup (security note: restrict after first run)
      console.log('[migrate-tables] ℹ️  Running migration without auth (one-time setup)')
    }

    console.log('[migrate-tables] 🔄 Starting migration...')

    const supabase = createClient()

    // Execute migration by splitting into statements
    const statements = migrationSQL
      .split(';')
      .map(s => s.trim())
      .filter(s => s.length > 0)

    console.log(`[migrate-tables] 📋 Total statements: ${statements.length}`)
    ...
    for (let i = 0; i < statements.length; i++) {
      ...
      try {
        // Execute via raw SQL - requires using the service role
        const { error } = await (supabase as any).rpc('exec_raw_sql', { sql: stmt + ';' })

        if (error) {
          if (error.message?.includes('already exists') || error.message?.includes('already been created')) {
            console.log(`[migrate-tables] ℹ️  [${i + 1}/${statements.length}] Skipped (already exists)`)
          } else {
            errorCount++
            errors.push(`Statement ${i + 1}: ${error.message}`)
            console.error(`[migrate-tables] ❌ [${i + 1}/${statements.length}] Error: ${error.message}`)
          }
        } else {
          successCount++
          console.log(`[migrate-tables] ✅ [${i + 1}/${statements.length}] Executed`)
        }
      } catch (err) {
        errorCount++
        const msg = err instanceof Error ? err.message : String(err)
        errors.push(`Statement ${i + 1}: ${msg}`)
        console.error(`[migrate-tables] ❌ [${i + 1}/${statements.length}] Error: ${msg}`)
      }
    }

    console.log(`[migrate-tables] 📊 Summary: ${successCount} executed, ${errorCount} errors`)

    if (errorCount === 0 || errorCount < statements.length / 2) {
      console.log('[migrate-tables] ✅ Migration completed!')
      ...
      console.error('[migrate-tables] ❌ Migration failed')
      return NextResponse.json(
        {
          success: false,
    ...
    console.error('[migrate-tables] ❌ Fatal error:', message)
    return NextResponse.json(
      { error: message },
      { status: 500 }
    )
